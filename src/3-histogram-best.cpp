...
struct histogram {
	...
	void set_add(int i, int v) { data[i] += v; }
...
void worker(int sample_count, vector<int> &h, int num_bins) {
	// long count = 0.0;
...
int main(int argc, char **argv) {
	...
	int print_level = 3; // 0: exec info + histogram total, 1: + histogram bins,
						 // 2: +exec time, 3: +bin info
